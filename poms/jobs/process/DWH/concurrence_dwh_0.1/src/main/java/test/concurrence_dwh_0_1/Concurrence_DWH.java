...
package test.concurrence_dwh_0_1;
...
/**
 * Job: Concurrence_DWH Purpose: <br>
 * Description: <br>
 * 
 ...
 * @status
 */
public class Concurrence_DWH implements TalendJob {
	...
	private final String jobName = "Concurrence_DWH";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					Concurrence_DWH.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(Concurrence_DWH.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class out1Struct implements routines.system.IPersistableRow<out1Struct> {
		final static byte[] commonByteArrayLock_TEST_Concurrence_DWH = new byte[0];
		static byte[] commonByteArray_TEST_Concurrence_DWH = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_TEST_Concurrence_DWH.length) {
					if (length < 1024 && commonByteArray_TEST_Concurrence_DWH.length == 0) {
						commonByteArray_TEST_Concurrence_DWH = new byte[1024];
					} else {
						commonByteArray_TEST_Concurrence_DWH = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_TEST_Concurrence_DWH, 0, length);
				strReturn = new String(commonByteArray_TEST_Concurrence_DWH, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_TEST_Concurrence_DWH, 0, length);
				strReturn = new String(commonByteArray_TEST_Concurrence_DWH, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_TEST_Concurrence_DWH) {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				final String decryptedPassword_tDBOutput_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:sU/5TQz75x9v31LNuK2hv5VFm8Cm0fqLtOb6v6sj");
				...
				if (dbschema_tDBOutput_1 == null || dbschema_tDBOutput_1.trim().length() == 0) {
					tableName_tDBOutput_1 = "Dim_Concurrence";
				} else {
					tableName_tDBOutput_1 = dbschema_tDBOutput_1 + "].[" + "Dim_Concurrence";
				}
				int count_tDBOutput_1 = 0;

				String insert_tDBOutput_1 = "INSERT INTO [" + tableName_tDBOutput_1
						+ "] ([category],[event_name],[event_date],[city],[country],[price]) VALUES (?,?,?,?,?,?)";
				...
				final String decryptedPassword_tDBOutput_2 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:we5DRLWdmbCwgEixKz52h+QtbPvDyOAnP1LEnrf3");
				...
				if (dbschema_tDBOutput_2 == null || dbschema_tDBOutput_2.trim().length() == 0) {
					tableName_tDBOutput_2 = "Dim_Concurrence";
				} else {
					tableName_tDBOutput_2 = dbschema_tDBOutput_2 + "].[" + "Dim_Concurrence";
				}
				int count_tDBOutput_2 = 0;

				String update_tDBOutput_2 = "UPDATE [" + tableName_tDBOutput_2
						+ "] SET [category] = ?,[event_name] = ?,[event_date] = ?,[city] = ?,[country] = ?,[price] = ? WHERE [id_concurrence] = ?";
...
// ###############################
				...
				final String decryptedPassword_tDBInput_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:tgiRroqEjePaVnyXo3vZ4RIt8no+uL10ZQAO26rJQQ==");
...
// # Filter conditions 
							...
							out1 == null || (row1.city == null ? out1.city != null : !row1.city.equals(out1.city))
									|| (row1.country == null ? out1.country != null
											: !row1.country.equals(out1.country))
									|| (row1.price == null ? out1.price != null : !row1.price.equals(out1.price))

							) {
								out1_tmp.id_concurrence = Numeric.sequence("seq1", 1, 1);
								out1_tmp.category = row1.category;
								out1_tmp.event_name = row1.event_name;
								out1_tmp.event_date = (row1.event_date == null || row1.event_date.trim().isEmpty())
										? "NaN"
										: row1.event_date.trim();
								out1_tmp.city = row1.city;
								out1_tmp.country = row1.country == null ? row1.city : row1.country;
...
// # Filter conditions 
							...
							out1 != null
									&& (row1.category == null ? out1.category == null
											: row1.category.equals(out1.category))
									&& (row1.event_name == null ? out1.event_name == null
											: row1.event_name.equals(out1.event_name))
									&& (row1.event_date == null ? out1.event_date == null
											: row1.event_date.equals(out1.event_date))

							) {
								update_tmp.id_concurrence = out1.id_concurrence;
								update_tmp.category = row1.category;
								update_tmp.event_name = row1.event_name;
								update_tmp.event_date = (row1.event_date == null || row1.event_date.trim().isEmpty())
										? "NaN"
										: row1.event_date.trim();
								update_tmp.city = row1.city;
								update_tmp.country = row1.country == null ? row1.city : row1.country;
								update_tmp.price = row1.price;
								update = update_tmp;
							} // closing filter/reject
// ###############################
...
// Start of branch "out1"
						if (out1 != null) {
							...
							if (out1.category == null) {
								...
								pstmt_tDBOutput_1.setString(1, out1.category);
							}

							if (out1.event_name == null) {
								pstmt_tDBOutput_1.setNull(2, java.sql.Types.VARCHAR);
							} else {
								pstmt_tDBOutput_1.setString(2, out1.event_name);
							}

							if (out1.event_date == null) {
								pstmt_tDBOutput_1.setNull(3, java.sql.Types.VARCHAR);
							} else {
								pstmt_tDBOutput_1.setString(3, out1.event_date);
							}
							...
							if (out1.price == null) {
								...
								pstmt_tDBOutput_1.setString(6, out1.price);
							}
...
// Start of branch "update"
						if (update != null) {
							...
							if (update.category == null) {
								...
								pstmt_tDBOutput_2.setString(1, update.category);
							}

							if (update.event_name == null) {
								pstmt_tDBOutput_2.setNull(2, java.sql.Types.VARCHAR);
							} else {
								pstmt_tDBOutput_2.setString(2, update.event_name);
							}

							if (update.event_date == null) {
								pstmt_tDBOutput_2.setNull(3, java.sql.Types.VARCHAR);
							} else {
								pstmt_tDBOutput_2.setString(3, update.event_date);
							}

							if (update.city == null) {
								pstmt_tDBOutput_2.setNull(4, java.sql.Types.VARCHAR);
							} else {
								pstmt_tDBOutput_2.setString(4, update.city);
							}

							if (update.country == null) {
								pstmt_tDBOutput_2.setNull(5, java.sql.Types.VARCHAR);
							} else {
								pstmt_tDBOutput_2.setString(5, update.country);
							}

							if (update.price == null) {
								pstmt_tDBOutput_2.setNull(6, java.sql.Types.VARCHAR);
							} else {
								pstmt_tDBOutput_2.setString(6, update.price);
							}

							if (update.id_concurrence == null) {
								pstmt_tDBOutput_2.setNull(7 + count_tDBOutput_2, java.sql.Types.INTEGER);
							} else {
								pstmt_tDBOutput_2.setInt(7 + count_tDBOutput_2, update.id_concurrence);
							}
...
// ###############################      
	...
	public static void main(String[] args) {
		final Concurrence_DWH Concurrence_DWHClass = new Concurrence_DWH();

		int exitCode = Concurrence_DWHClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = Concurrence_DWH.class.getClassLoader()
					.getResourceAsStream("test/concurrence_dwh_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = Concurrence_DWH.class.getClassLoader()
		...
		if (false) {
			System.out.println(
					(endUsedMemory - startUsedMemory) + " bytes memory increase when running : Concurrence_DWH");
		}
...
/************************************************************************************************
 * 87534 characters generated by Talend Open Studio for Data Integration on the
 * 2 mars 2026, 00:31:16 WAT
 ************************************************************************************************/