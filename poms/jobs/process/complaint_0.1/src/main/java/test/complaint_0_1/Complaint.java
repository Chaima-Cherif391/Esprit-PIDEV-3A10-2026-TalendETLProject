 ...
 * @status
 */
public class Complaint implements TalendJob {
	...
	private final String jobName = "Complaint";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(Complaint.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
		final static byte[] commonByteArrayLock_TEST_Complaint = new byte[0];
		static byte[] commonByteArray_TEST_Complaint = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_TEST_Complaint.length) {
					if (length < 1024 && commonByteArray_TEST_Complaint.length == 0) {
						commonByteArray_TEST_Complaint = new byte[1024];
					} else {
						commonByteArray_TEST_Complaint = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_TEST_Complaint, 0, length);
				strReturn = new String(commonByteArray_TEST_Complaint, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_TEST_Complaint, 0, length);
				strReturn = new String(commonByteArray_TEST_Complaint, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_TEST_Complaint) {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				final String decryptedPassword_tDBOutput_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:N5GWFBA12Dzf39VC1ldatK1tzqOtLbtJ2ALiOidMFg==");
				...
				if (dbschema_tDBOutput_1 == null || dbschema_tDBOutput_1.trim().length() == 0) {
					tableName_tDBOutput_1 = "Complaint";
				} else {
					tableName_tDBOutput_1 = dbschema_tDBOutput_1 + "].[" + "Complaint";
				}
				...
				String insert_tDBOutput_1 = "INSERT INTO [" + tableName_tDBOutput_1
						+ "] ([id_complaint],[description],[status],[id_beneficiary],[id_provider],[subject]) VALUES (?,?,?,?,?,?)";
				...
				final String decryptedPassword_tFileInputExcel_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:jmGiVSfeibK8HcHQ+mPWobtaFqNqiuFouZeFdw==");
				...
				try {
					...
					sheetList_tFileInputExcel_1
							.addAll(regexUtil_tFileInputExcel_1.getSheets(workbook_tFileInputExcel_1, 8, false));
						...
						for (int i_tFileInputExcel_1 = begin_line_tFileInputExcel_1; i_tFileInputExcel_1 < end_line_tFileInputExcel_1; i_tFileInputExcel_1++) {
							...
							try {
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									...
									row1.id_complaint = ParserUtils.parseTo_Integer(ParserUtils.parseTo_Number(
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									...
									row1.id_provider = ParserUtils.parseTo_Integer(ParserUtils.parseTo_Number(
											temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1], null,
											'.' == decimalChar_tFileInputExcel_1 ? null
													: decimalChar_tFileInputExcel_1));
								} else {
...
// Start of branch "row1"
							if (row1 != null) {
								...
								if (row1.id_complaint == null) {
									...
									pstmt_tDBOutput_1.setInt(1, row1.id_complaint);
								}
	...
	public static void main(String[] args) {
		final Complaint ComplaintClass = new Complaint();

		int exitCode = ComplaintClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = Complaint.class.getClassLoader()
					.getResourceAsStream("test/complaint_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = Complaint.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : Complaint");
		}
...
/************************************************************************************************
 * 61765 characters generated by Talend Open Studio for Data Integration on the
 * 28 janvier 2026, 11:44:59 WAT
 ************************************************************************************************/