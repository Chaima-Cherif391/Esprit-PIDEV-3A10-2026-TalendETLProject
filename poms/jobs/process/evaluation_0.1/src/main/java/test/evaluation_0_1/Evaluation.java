 ...
 * @status
 */
public class Evaluation implements TalendJob {
	...
	private final String jobName = "Evaluation";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(Evaluation.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
		final static byte[] commonByteArrayLock_TEST_Evaluation = new byte[0];
		static byte[] commonByteArray_TEST_Evaluation = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_TEST_Evaluation.length) {
					if (length < 1024 && commonByteArray_TEST_Evaluation.length == 0) {
						commonByteArray_TEST_Evaluation = new byte[1024];
					} else {
						commonByteArray_TEST_Evaluation = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_TEST_Evaluation, 0, length);
				strReturn = new String(commonByteArray_TEST_Evaluation, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_TEST_Evaluation, 0, length);
				strReturn = new String(commonByteArray_TEST_Evaluation, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_TEST_Evaluation) {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				final String decryptedPassword_tDBOutput_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:o5+/i3PaNpP8BqsQ1GYE5YpW7uqRVnpg3ou/VkGocw==");
				...
				if (dbschema_tDBOutput_1 == null || dbschema_tDBOutput_1.trim().length() == 0) {
					tableName_tDBOutput_1 = "Evaluation";
				} else {
					tableName_tDBOutput_1 = dbschema_tDBOutput_1 + "].[" + "Evaluation";
				}
				...
				String insert_tDBOutput_1 = "INSERT INTO [" + tableName_tDBOutput_1
						+ "] ([id_evaluation],[id_reservation],[rating],[comment]) VALUES (?,?,?,?)";
				...
				final String decryptedPassword_tFileInputExcel_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:scVJPyNq88FDOy6BWK9VhRcosEx4lbpfzdmBZw==");
				...
				try {
					...
					sheetList_tFileInputExcel_1
							.addAll(regexUtil_tFileInputExcel_1.getSheets(workbook_tFileInputExcel_1, 7, false));
						...
						for (int i_tFileInputExcel_1 = begin_line_tFileInputExcel_1; i_tFileInputExcel_1 < end_line_tFileInputExcel_1; i_tFileInputExcel_1++) {
							...
							try {
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									...
									row1.id_evaluation = ParserUtils.parseTo_Integer(ParserUtils.parseTo_Number(
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									...
									row1.id_reservation = ParserUtils.parseTo_Integer(ParserUtils.parseTo_Number(
											temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1], null,
											'.' == decimalChar_tFileInputExcel_1 ? null
													: decimalChar_tFileInputExcel_1));
								} else {
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									...
									row1.rating = ParserUtils.parseTo_Integer(ParserUtils.parseTo_Number(
											temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1], null,
											'.' == decimalChar_tFileInputExcel_1 ? null
													: decimalChar_tFileInputExcel_1));
								} else {
...
// Start of branch "row1"
							if (row1 != null) {
								...
								if (row1.id_evaluation == null) {
									...
									pstmt_tDBOutput_1.setInt(1, row1.id_evaluation);
								}

								if (row1.id_reservation == null) {
									pstmt_tDBOutput_1.setNull(2, java.sql.Types.INTEGER);
								} else {
									pstmt_tDBOutput_1.setInt(2, row1.id_reservation);
								}

								if (row1.rating == null) {
									pstmt_tDBOutput_1.setNull(3, java.sql.Types.INTEGER);
								} else {
									pstmt_tDBOutput_1.setInt(3, row1.rating);
								}

								if (row1.comment == null) {
									pstmt_tDBOutput_1.setNull(4, java.sql.Types.VARCHAR);
								} else {
									pstmt_tDBOutput_1.setString(4, row1.comment);
								}
	...
	public static void main(String[] args) {
		final Evaluation EvaluationClass = new Evaluation();

		int exitCode = EvaluationClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = Evaluation.class.getClassLoader()
					.getResourceAsStream("test/evaluation_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = Evaluation.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : Evaluation");
		}
...
/************************************************************************************************
 * 59712 characters generated by Talend Open Studio for Data Integration on the
 * 28 janvier 2026, 11:45:39 WAT
 ************************************************************************************************/