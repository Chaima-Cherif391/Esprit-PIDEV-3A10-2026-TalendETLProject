 ...
 * @status
 */
public class evenement implements TalendJob {
	...
	private final String jobName = "evenement";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(evenement.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
		final static byte[] commonByteArrayLock_TEST_evenement = new byte[0];
		static byte[] commonByteArray_TEST_evenement = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_TEST_evenement.length) {
					if (length < 1024 && commonByteArray_TEST_evenement.length == 0) {
						commonByteArray_TEST_evenement = new byte[1024];
					} else {
						commonByteArray_TEST_evenement = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_TEST_evenement, 0, length);
				strReturn = new String(commonByteArray_TEST_evenement, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_TEST_evenement, 0, length);
				strReturn = new String(commonByteArray_TEST_evenement, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_TEST_evenement) {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				final String decryptedPassword_tDBOutput_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:hC1W6lb0mYQ2MTK5FDZeO8L7+BU6Q6hVdD0SZzU6Ow==");
				...
				if (dbschema_tDBOutput_1 == null || dbschema_tDBOutput_1.trim().length() == 0) {
					tableName_tDBOutput_1 = "Event";
				} else {
					tableName_tDBOutput_1 = dbschema_tDBOutput_1 + "].[" + "Event";
				}
				...
				String insert_tDBOutput_1 = "INSERT INTO [" + tableName_tDBOutput_1
						+ "] ([id_event],[title],[event_date],[budget],[type],[id_beneficiary]) VALUES (?,?,?,?,?,?)";
				...
				final String decryptedPassword_tFileInputExcel_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:Y6boweeiEGYr/DBimNMbCwILzFEfnp5w2CwIuw==");
				...
				Object source_tFileInputExcel_1 = "C:/Users/user/OneDrive - ESPRIT/Bureau/4BI/PA_BI/eventzella_schema.xlsx";
				...
				try {
					...
					sheetList_tFileInputExcel_1
							.addAll(regexUtil_tFileInputExcel_1.getSheets(workbook_tFileInputExcel_1, 5, false));
						...
						for (int i_tFileInputExcel_1 = begin_line_tFileInputExcel_1; i_tFileInputExcel_1 < end_line_tFileInputExcel_1; i_tFileInputExcel_1++) {
							...
							try {
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									...
									row1.id_event = ParserUtils.parseTo_Integer(ParserUtils.parseTo_Number(
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									curColNum_tFileInputExcel_1 = columnIndex_tFileInputExcel_1
											+ start_column_tFileInputExcel_1 + 1;
									curColName_tFileInputExcel_1 = "budget";

									row1.budget = ParserUtils.parseTo_Integer(ParserUtils.parseTo_Number(
											temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1], null,
											'.' == decimalChar_tFileInputExcel_1 ? null
													: decimalChar_tFileInputExcel_1));
								} else {
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									...
									row1.id_beneficiary = ParserUtils.parseTo_Integer(ParserUtils.parseTo_Number(
											temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1], null,
											'.' == decimalChar_tFileInputExcel_1 ? null
													: decimalChar_tFileInputExcel_1));
								} else {
...
// Start of branch "row1"
							if (row1 != null) {
								...
								if (row1.id_event == null) {
									...
									pstmt_tDBOutput_1.setInt(1, row1.id_event);
								}

								if (row1.title == null) {
									pstmt_tDBOutput_1.setNull(2, java.sql.Types.VARCHAR);
								} else {
									pstmt_tDBOutput_1.setString(2, row1.title);
								}
								...
								if (row1.budget == null) {
									...
									pstmt_tDBOutput_1.setInt(4, row1.budget);
								}

								if (row1.type == null) {
									pstmt_tDBOutput_1.setNull(5, java.sql.Types.VARCHAR);
								} else {
									pstmt_tDBOutput_1.setString(5, row1.type);
								}

								if (row1.id_beneficiary == null) {
									pstmt_tDBOutput_1.setNull(6, java.sql.Types.INTEGER);
								} else {
									pstmt_tDBOutput_1.setInt(6, row1.id_beneficiary);
								}
	...
	public static void main(String[] args) {
		final evenement evenementClass = new evenement();

		int exitCode = evenementClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = evenement.class.getClassLoader()
					.getResourceAsStream("test/evenement_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = evenement.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : evenement");
		}
...
/************************************************************************************************
 * 61547 characters generated by Talend Open Studio for Data Integration on the
 * 28 janvier 2026, 11:46:04 WAT
 ************************************************************************************************/