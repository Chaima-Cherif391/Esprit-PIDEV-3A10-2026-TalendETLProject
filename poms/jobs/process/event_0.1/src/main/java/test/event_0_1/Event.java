 ...
 * @status
 */
public class Event implements TalendJob {
	...
	private final String jobName = "Event";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(Event.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
		final static byte[] commonByteArrayLock_TEST_Event = new byte[0];
		static byte[] commonByteArray_TEST_Event = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_TEST_Event.length) {
					if (length < 1024 && commonByteArray_TEST_Event.length == 0) {
						commonByteArray_TEST_Event = new byte[1024];
					} else {
						commonByteArray_TEST_Event = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_TEST_Event, 0, length);
				strReturn = new String(commonByteArray_TEST_Event, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_TEST_Event, 0, length);
				strReturn = new String(commonByteArray_TEST_Event, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_TEST_Event) {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				final String decryptedPassword_tDBOutput_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:f1QikigQuKlP3rkblkWh+uCnz4GTYYAC1i+0VNCKaA==");
				...
				if (dbschema_tDBOutput_1 == null || dbschema_tDBOutput_1.trim().length() == 0) {
					tableName_tDBOutput_1 = "Beneficiary";
				} else {
					tableName_tDBOutput_1 = dbschema_tDBOutput_1 + "].[" + "Beneficiary";
				}
				...
				String insert_tDBOutput_1 = "INSERT INTO [" + tableName_tDBOutput_1
						+ "] ([id_beneficiary],[first_name],[last_name],[email],[phone]) VALUES (?,?,?,?,?)";
				...
				final String decryptedPassword_tFileInputExcel_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:sMNMHCynKYwTaGOg0UKRdL6zmbo1S7nx2or6ZA==");
				...
				try {
					...
					sheetList_tFileInputExcel_1
							.addAll(regexUtil_tFileInputExcel_1.getSheets(workbook_tFileInputExcel_1, 0, false));
...
// Start of branch "row1"
							if (row1 != null) {
								...
								if (row1.id_beneficiary == null) {
									...
									pstmt_tDBOutput_1.setInt(1, row1.id_beneficiary);
								}
	...
	public static void main(String[] args) {
		final Event EventClass = new Event();

		int exitCode = EventClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = Event.class.getClassLoader()
					.getResourceAsStream("test/event_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = Event.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : Event");
		}
...
/************************************************************************************************
 * 60172 characters generated by Talend Open Studio for Data Integration on the
 * 28 janvier 2026, 11:46:39 WAT
 ************************************************************************************************/