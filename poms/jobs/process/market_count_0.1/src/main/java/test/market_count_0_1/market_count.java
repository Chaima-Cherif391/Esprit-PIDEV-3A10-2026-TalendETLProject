 ...
 * @status
 */
public class market_count implements TalendJob {
	...
	private final String jobName = "market_count";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(market_count.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
		final static byte[] commonByteArrayLock_TEST_market_count = new byte[0];
		static byte[] commonByteArray_TEST_market_count = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_TEST_market_count.length) {
					if (length < 1024 && commonByteArray_TEST_market_count.length == 0) {
						commonByteArray_TEST_market_count = new byte[1024];
					} else {
						commonByteArray_TEST_market_count = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_TEST_market_count, 0, length);
				strReturn = new String(commonByteArray_TEST_market_count, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_TEST_market_count, 0, length);
				strReturn = new String(commonByteArray_TEST_market_count, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_TEST_market_count) {
				...
				try {
					...
					this.market_count_googlemaps = readInteger(dis);
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				writeInteger(this.market_count_googlemaps, dos);
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				final String decryptedPassword_tDBOutput_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:LV+VfuyqjiAnZ7NGNdxkVwIkfhgtIEA53j25l/7CiQ==");
				...
				if (dbschema_tDBOutput_1 == null || dbschema_tDBOutput_1.trim().length() == 0) {
					tableName_tDBOutput_1 = "marketCount";
				} else {
					tableName_tDBOutput_1 = dbschema_tDBOutput_1 + "].[" + "marketCount";
				}
				...
				if (!whetherExist_tDBOutput_1) {
					try (java.sql.Statement stmtCreate_tDBOutput_1 = conn_tDBOutput_1.createStatement()) {
						stmtCreate_tDBOutput_1.execute("CREATE TABLE [" + tableName_tDBOutput_1
								+ "]([id_category] INT ,[id_subcategory] INT ,[ville] VARCHAR(9)  ,[market_count_googlemaps] INT ,[Category] VARCHAR(9)  ,[Service] VARCHAR(13)  )");
					}
				}
				String insert_tDBOutput_1 = "INSERT INTO [" + tableName_tDBOutput_1
						+ "] ([id_category],[id_subcategory],[ville],[market_count_googlemaps],[Category],[Service]) VALUES (?,?,?,?,?,?)";
				...
				final String decryptedPassword_tFileInputExcel_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:Uc7aSHzZrEnOYfsYnf2GqvAZilSiL+kkXtThZw==");
				...
				Object source_tFileInputExcel_1 = "C:/Users/user/Downloads/eventzella_schema (4).xlsx";
				...
				try {
					...
					sheetList_tFileInputExcel_1.addAll(
							regexUtil_tFileInputExcel_1.getSheets(workbook_tFileInputExcel_1, "MARKET-COUNT", false));
						...
						for (int i_tFileInputExcel_1 = begin_line_tFileInputExcel_1; i_tFileInputExcel_1 < end_line_tFileInputExcel_1; i_tFileInputExcel_1++) {
							...
							try {
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									...
									row1.id_category = ParserUtils.parseTo_Integer(ParserUtils.parseTo_Number(
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									...
									row1.id_subcategory = ParserUtils.parseTo_Integer(ParserUtils.parseTo_Number(
											temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1], null,
											'.' == decimalChar_tFileInputExcel_1 ? null
													: decimalChar_tFileInputExcel_1));
								} else {
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									curColNum_tFileInputExcel_1 = columnIndex_tFileInputExcel_1
											+ start_column_tFileInputExcel_1 + 1;
									curColName_tFileInputExcel_1 = "market_count_googlemaps";

									row1.market_count_googlemaps = ParserUtils.parseTo_Integer(ParserUtils
											.parseTo_Number(temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1],
													null, '.' == decimalChar_tFileInputExcel_1 ? null
															: decimalChar_tFileInputExcel_1));
								} else {
									row1.market_count_googlemaps = null;
									emptyColumnCount_tFileInputExcel_1++;
								}
...
// Start of branch "row1"
							if (row1 != null) {
								...
								if (row1.id_category == null) {
									...
									pstmt_tDBOutput_1.setInt(1, row1.id_category);
								}

								if (row1.id_subcategory == null) {
									pstmt_tDBOutput_1.setNull(2, java.sql.Types.INTEGER);
								} else {
									pstmt_tDBOutput_1.setInt(2, row1.id_subcategory);
								}

								if (row1.ville == null) {
									pstmt_tDBOutput_1.setNull(3, java.sql.Types.VARCHAR);
								} else {
									pstmt_tDBOutput_1.setString(3, row1.ville);
								}

								if (row1.market_count_googlemaps == null) {
									pstmt_tDBOutput_1.setNull(4, java.sql.Types.INTEGER);
								} else {
									pstmt_tDBOutput_1.setInt(4, row1.market_count_googlemaps);
								}

								if (row1.Category == null) {
									pstmt_tDBOutput_1.setNull(5, java.sql.Types.VARCHAR);
								} else {
									pstmt_tDBOutput_1.setString(5, row1.Category);
								}

								if (row1.Service == null) {
									pstmt_tDBOutput_1.setNull(6, java.sql.Types.VARCHAR);
								} else {
									pstmt_tDBOutput_1.setString(6, row1.Service);
								}
	...
	public static void main(String[] args) {
		final market_count market_countClass = new market_count();

		int exitCode = market_countClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = market_count.class.getClassLoader()
					.getResourceAsStream("test/market_count_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = market_count.class.getClassLoader()
		...
		if (false) {
			System.out
					.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : market_count");
		}
...
/************************************************************************************************
 * 62989 characters generated by Talend Open Studio for Data Integration on the
 * 8 février 2026, 19:29:33 WAT
 ************************************************************************************************/