 ...
 * @status
 */
public class Provider implements TalendJob {
	...
	private final String jobName = "Provider";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(Provider.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
		final static byte[] commonByteArrayLock_TEST_Provider = new byte[0];
		static byte[] commonByteArray_TEST_Provider = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_TEST_Provider.length) {
					if (length < 1024 && commonByteArray_TEST_Provider.length == 0) {
						commonByteArray_TEST_Provider = new byte[1024];
					} else {
						commonByteArray_TEST_Provider = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_TEST_Provider, 0, length);
				strReturn = new String(commonByteArray_TEST_Provider, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_TEST_Provider, 0, length);
				strReturn = new String(commonByteArray_TEST_Provider, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_TEST_Provider) {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				final String decryptedPassword_tDBOutput_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:kZLXEjcFZL3fzeTPYm9r5nqJFcktinrJrdyDX8V7rA==");
				...
				if (dbschema_tDBOutput_1 == null || dbschema_tDBOutput_1.trim().length() == 0) {
					tableName_tDBOutput_1 = "Provider";
				} else {
					tableName_tDBOutput_1 = dbschema_tDBOutput_1 + "].[" + "Provider";
				}
				...
				String insert_tDBOutput_1 = "INSERT INTO [" + tableName_tDBOutput_1
						+ "] ([id_provider],[name],[service_type],[email],[phone],[city]) VALUES (?,?,?,?,?,?)";
				...
				final String decryptedPassword_tFileInputExcel_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:9zEKDmF2RIFWorh1IKTl4mrpY7O4xCLbro/6tg==");
				...
				try {
					...
					sheetList_tFileInputExcel_1
							.addAll(regexUtil_tFileInputExcel_1.getSheets(workbook_tFileInputExcel_1, 1, false));
...
// Start of branch "row1"
							if (row1 != null) {
								...
								if (row1.id_provider == null) {
									...
									pstmt_tDBOutput_1.setInt(1, row1.id_provider);
								}
								...
								if (row1.service_type == null) {
									...
									pstmt_tDBOutput_1.setString(3, row1.service_type);
								}
								...
								if (row1.phone == null) {
									...
									pstmt_tDBOutput_1.setString(5, row1.phone);
								}
	...
	public static void main(String[] args) {
		final Provider ProviderClass = new Provider();

		int exitCode = ProviderClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = Provider.class.getClassLoader()
					.getResourceAsStream("test/provider_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = Provider.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : Provider");
		}
...
/************************************************************************************************
 * 61111 characters generated by Talend Open Studio for Data Integration on the
 * 28 janvier 2026, 11:47:59 WAT
 ************************************************************************************************/