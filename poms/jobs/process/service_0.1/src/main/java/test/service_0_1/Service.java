...
public class Service implements TalendJob {
	...
	public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
		final static byte[] commonByteArrayLock_TEST_Service = new byte[0];
		static byte[] commonByteArray_TEST_Service = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_TEST_Service.length) {
					if (length < 1024 && commonByteArray_TEST_Service.length == 0) {
						commonByteArray_TEST_Service = new byte[1024];
					} else {
						commonByteArray_TEST_Service = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_TEST_Service, 0, length);
				strReturn = new String(commonByteArray_TEST_Service, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_TEST_Service, 0, length);
				strReturn = new String(commonByteArray_TEST_Service, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_TEST_Service) {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				final String decryptedPassword_tDBOutput_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:cegJL1gDwnNiFRNXKzyDzXt7B6GEFvzdrJGcvFwGDA==");
				...
				if (dbschema_tDBOutput_1 == null || dbschema_tDBOutput_1.trim().length() == 0) {
					tableName_tDBOutput_1 = "Service";
				} else {
					tableName_tDBOutput_1 = dbschema_tDBOutput_1 + "].[" + "Service";
				}
				...
				String insert_tDBOutput_1 = "INSERT INTO [" + tableName_tDBOutput_1
						+ "] ([id_service],[price],[description],[id_provider],[id_subcategory],[title]) VALUES (?,?,?,?,?,?)";
				...
				final String decryptedPassword_tFileInputExcel_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:ykq6Abf4adwSWd7AYxe/d2bEVqzBSy14YGgBHg==");
				...
				try {
					...
					sheetList_tFileInputExcel_1
							.addAll(regexUtil_tFileInputExcel_1.getSheets(workbook_tFileInputExcel_1, 4, false));
						...
						for (int i_tFileInputExcel_1 = begin_line_tFileInputExcel_1; i_tFileInputExcel_1 < end_line_tFileInputExcel_1; i_tFileInputExcel_1++) {
							...
							try {
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									...
									row1.id_service = ParserUtils.parseTo_Integer(ParserUtils.parseTo_Number(
								...
								if (temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1].length() > 0) {
									...
									row1.price = ParserUtils.parseTo_Integer(ParserUtils.parseTo_Number(
											temp_row_tFileInputExcel_1[columnIndex_tFileInputExcel_1], null,
											'.' == decimalChar_tFileInputExcel_1 ? null
													: decimalChar_tFileInputExcel_1));
								} else {
...
// Start of branch "row1"
							if (row1 != null) {
								...
								if (row1.id_service == null) {
									...
									pstmt_tDBOutput_1.setInt(1, row1.id_service);
								}

								if (row1.price == null) {
									pstmt_tDBOutput_1.setNull(2, java.sql.Types.INTEGER);
								} else {
									pstmt_tDBOutput_1.setInt(2, row1.price);
								}

								if (row1.description == null) {
									pstmt_tDBOutput_1.setNull(3, java.sql.Types.VARCHAR);
								} else {
									pstmt_tDBOutput_1.setString(3, row1.description);
								}

								if (row1.id_provider == null) {
									pstmt_tDBOutput_1.setNull(4, java.sql.Types.INTEGER);
								} else {
									pstmt_tDBOutput_1.setInt(4, row1.id_provider);
								}

								if (row1.id_subcategory == null) {
									pstmt_tDBOutput_1.setNull(5, java.sql.Types.INTEGER);
								} else {
									pstmt_tDBOutput_1.setInt(5, row1.id_subcategory);
								}

								if (row1.title == null) {
									pstmt_tDBOutput_1.setNull(6, java.sql.Types.VARCHAR);
								} else {
									pstmt_tDBOutput_1.setString(6, row1.title);
								}
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = Service.class.getClassLoader()
					.getResourceAsStream("test/service_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = Service.class.getClassLoader()
...
/************************************************************************************************
 * 61787 characters generated by Talend Open Studio for Data Integration on the
 * 28 janvier 2026, 11:49:08 WAT
 ************************************************************************************************/