 ...
 * @status
 */
public class SubCategory implements TalendJob {
	...
	private final String jobName = "SubCategory";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(SubCategory.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
		final static byte[] commonByteArrayLock_TEST_SubCategory = new byte[0];
		static byte[] commonByteArray_TEST_SubCategory = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_TEST_SubCategory.length) {
					if (length < 1024 && commonByteArray_TEST_SubCategory.length == 0) {
						commonByteArray_TEST_SubCategory = new byte[1024];
					} else {
						commonByteArray_TEST_SubCategory = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_TEST_SubCategory, 0, length);
				strReturn = new String(commonByteArray_TEST_SubCategory, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_TEST_SubCategory, 0, length);
				strReturn = new String(commonByteArray_TEST_SubCategory, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_TEST_SubCategory) {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				final String decryptedPassword_tDBOutput_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:QibKBGnSXmR3ymeCLCWhMcqLzMgKkA6COELnFSxg9Q==");
				...
				if (dbschema_tDBOutput_1 == null || dbschema_tDBOutput_1.trim().length() == 0) {
					tableName_tDBOutput_1 = "Subcategory";
				} else {
					tableName_tDBOutput_1 = dbschema_tDBOutput_1 + "].[" + "Subcategory";
				}
				...
				String insert_tDBOutput_1 = "INSERT INTO [" + tableName_tDBOutput_1
						+ "] ([id_subcategory],[name],[id_category]) VALUES (?,?,?)";
				...
				final String decryptedPassword_tFileInputExcel_1 = routines.system.PasswordEncryptUtil
						.decryptPassword("enc:routine.encryption.key.v1:OEhqk3Mgyi65c2vjvSfUKdV14LrmGrpsjrf+6g==");
				...
				try {
					...
					sheetList_tFileInputExcel_1
							.addAll(regexUtil_tFileInputExcel_1.getSheets(workbook_tFileInputExcel_1, 3, false));
...
// Start of branch "row1"
							if (row1 != null) {
								...
								if (row1.id_subcategory == null) {
									...
									pstmt_tDBOutput_1.setInt(1, row1.id_subcategory);
								}
								...
								if (row1.id_category == null) {
									...
									pstmt_tDBOutput_1.setInt(3, row1.id_category);
								}
	...
	public static void main(String[] args) {
		final SubCategory SubCategoryClass = new SubCategory();

		int exitCode = SubCategoryClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = SubCategory.class.getClassLoader()
					.getResourceAsStream("test/subcategory_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = SubCategory.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : SubCategory");
		}
...
/************************************************************************************************
 * 58533 characters generated by Talend Open Studio for Data Integration on the
 * 28 janvier 2026, 11:49:39 WAT
 ************************************************************************************************/